export default {
  async fetch(request, env) {
    ...
    try {
      const { prompt, references } = await request.json();
      if (!prompt) return json({ error: "Prompt is required" }, 400);

      // 🧠 Call Flux 2 Dev model
      const result = await env.AI.run("@cf/black-forest-labs/flux-2-dev", { 
        prompt, 
        references // optional array of reference images for multi-reference editing
      });
      ...
      if (typeof result === "string") {
        try {
          ...
          base64Image = parsed.image || parsed.data; 
        } catch {
          base64Image = result; 
        }
