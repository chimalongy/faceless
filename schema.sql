...
-- Story Background Music table
create table story_background_music (
  id uuid primary key default uuid_generate_v4(),
  user_id uuid references users(id) on delete cascade not null,
  story_id uuid references stories(id) on delete cascade not null,
  music_url text not null,                  -- URL or path to the music file
  music_format text default 'mp3',         -- mp3, wav, etc.
  volume_level numeric default 0.5,        -- 0.0 to 1.0, default 50%
  scene_number integer default null,       -- optional: specific scene this music is for
  is_looping boolean default true,         -- whether the music should loop
  duration_seconds numeric default null,   -- optional: length of music in seconds
