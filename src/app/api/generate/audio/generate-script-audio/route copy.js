...
export async function POST(request) {
  try {
    const { storyId, audio_generation_link } = await request.json();
    ...
    for (const scene of scenes) {
     ...
     const audio_generation_result = await downloadAndUploadToSupabase(
      result.data[0].url,
      destinationPath,
    );

    console.log(audio_generation_result)
   
     if (!audio_generation_result.success){
      return NextResponse.json(
      ...
      const { error: insertError } = await supabase
        ...
        .insert({
          story_id: story.id,
          audio_url: audio_generation_result.url,
      ...
      storedAudioUrls.push(audio_generation_result.url);
    }
